...
function Settings() {
  const { blacklist, toggleBlacklist, maxPool, setMaxPool, availableCount } = useSavedLessons();
            ...
            <div className="border border-gray-500 rounded-xl p-4 bg-gray-900/40">
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  min={1}
                  max={maxAllowed}
                  value={Math.min(maxPool, maxAllowed)}
                ...
                <input
                  ...
                  value={Math.min(maxPool, maxAllowed)}
                  onChange={e => {
                    const val = Math.min(Math.max(1, Number(e.target.value)), maxAllowed);
                    setMaxPool(val);
                  }}
                  className="w-16 text-center rounded border border-gray-500 bg-gray-800 text-white p-1"
                />
              </div>
            </div>
            <div className="border border-gray-500 rounded-xl p-4 bg-gray-900/40">
